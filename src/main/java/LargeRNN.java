...
public class LargeRNN
{
    ...
    */
    ...
    {
        int lstmLayerSize = 3000;					//Number of units in 1st and 2nd GravesLSTM layer
        int batchsize = 50;						    //Size of mini batch to use when  training
        int nEpoches = 100;							//Total number of training epoch
        ...
        MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder()
                ...
                .layer(2, new GravesLSTM.Builder().nIn(lstmLayerSize).nOut(lstmLayerSize)
                        .activation(Activation.TANH).build())
                .layer(3, new GravesLSTM.Builder().nIn(lstmLayerSize).nOut(lstmLayerSize-1000)
                        .activation(Activation.TANH).build())
                .layer(4, new GravesLSTM.Builder().nIn(lstmLayerSize-1000).nOut(lstmLayerSize-2000)
                        .activation(Activation.TANH).build())
                .layer(5, new RnnOutputLayer.Builder(LossFunctions.LossFunction.MCXENT).activation(Activation.SOFTMAX)        //MCXENT + softmax for classification
                        .nIn(lstmLayerSize-2000).nOut(nOut).build())
